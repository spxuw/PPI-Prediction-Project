...
def generate_condition_and_linking_set(N_Mod_base, N_Em_base, N_Ad_base, N_Ad_plus_1,
    ...
    if __name__ == "__main__":
        ...
        with mp.Pool(processes=n_workers) as pool:
            main_data.extend(pool.starmap(handle_module, zip(it.repeat(N_Mod_base),it.repeat(N_Em_base), it.repeat(N_Ad_base), it.repeat(N_Ad_plus_1),it.repeat(module_th_low), it.repeat(comb_per_module), range(len(N_Mod_base)))))
        print(main_data)
