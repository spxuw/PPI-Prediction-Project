...
N81_Em = pd.read_csv(input_settings["oSourceList"]["oN81_Em"],
                        delimiter=' ', header = None, index_col= 0)
...
def build_discriminator(module_dim):
    ...
    merge = Concatenate()([condition, linking])
    ...
    model = Model([condition_input, linking_input], out_layer)
...
def build_generator(module_dim):
    ...
    e1 = define_encoder_block(condition, 64)
    ...
    model = Model(condition_input, out_layer)
...
def generate_fake_samples(g_model, conditions, base_adjacency):        
    ...
    linkings = g_model.predict(conditions)

    return linkings 

def calculate_gradient_penalty(d_model, conditions, real_sample, fake_sample, batch_num):
    ...
    with tf.GradientTape() as gtape:
        ...
        d_interpolate_output = d_model([conditions, interpolate_sample], training=True)
...
def train_discriminator(d_model, g_model, batch_num, d_iter,
    ...
    for i in range(d_iter):
        ...
        with tf.GradientTape() as gtape:
            ...
            g_output = g_model(conditions, training = True)
            d_fake_output = d_model([conditions, g_output], training=True)
            d_real_output = d_model([conditions, linkings], training=True)
            
            d_cost = calculate_discriminator_loss(d_real_output, d_fake_output)
            gp = calculate_gradient_penalty(d_model, conditions, linkings, g_output, batch_num)
...
def generate_results(g_model, data_test):
    ...
    expected_linkings_test = np.asarray(data_test_n[:, 3, :, :]).astype('float32')
    data_test_n = None
...
train_data = generate_condition_and_linking_set(N81_Mod, N81_Em, N81_Ad, 
                                                N90_Ad, module_size, recomb_per_module)

test_data = generate_condition_and_linking_set(N90_Mod, N90_Em, N90_Ad, 
