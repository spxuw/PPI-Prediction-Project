...
do
    ...
    python3 -u preprocess_n80_n64.py $network_name $input_dir/$network_name.csv $network_output_dir/$network_name/
    ...
    do
        ...
        then
    		...
    		for ((j=1; j<=5; j++)); do
    			...
    			sed -i 's,|2|,'$network_output_dir'/'$network_name'/'$species'_n80_train_kf_'$j'.edgelist,g' $json_path
    			sed -i 's,|3|,'$network_output_dir'/'$network_name'/'$species'_n80_modules_kf_'$j'.csv,g' $json_path
    			sed -i 's,|4|,'$network_output_dir'/'$network_name'/'$species'_n64_train_kf_'$j'.edgelist,g' $json_path
    			sed -i 's,|5|,'$network_output_dir'/'$network_name'/'$species'_n64_modules_kf_'$j'.csv,g' $json_path
