...
for train_index, test_index in kf.split(X):
    ...
    train_custom, test_custom = train_test_split(train, test_size=0.2, random_state=42, shuffle=True)

    n90 = analyse(train, test, kf_count, '_n80_', G)
    ncustom = analyse(train_custom, test_custom, kf_count, '_n64_', G)
...
net_coverage_n90_df.to_csv(output_dir+str(network_name)+'_n80_coverage_'+'.csv', \
    index=False, sep='\t')

net_coverage_n81_df = pd.DataFrame(net_coverage_ncustom, columns=['coverage'])
net_coverage_n81_df.to_csv(output_dir+str(network_name)+'_n64_coverage_'+ \
    '.csv', index=False, sep='\t')
